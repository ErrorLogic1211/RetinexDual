...
CUDA_VISIBLE_DEVICES=$gpu_ids python -m torch.distributed.run --nproc_per_node=$gpu_count --master_port=$master_port -m basicsr.train -opt $config --launcher pytorch